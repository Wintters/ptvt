...
def cambia_vista_m3u(modo):  # by DarioMO para cambios de modo de vista en listas m3u
	##Lo repito 3 veces xq el Kodi (o el Confluence) es cabezón y hay veces q necesita q se le insista
	if modo.upper() == "TV_SHOWS":
		plugintools.set_view(plugintools.TV_SHOWS)
		plugintools.set_view(plugintools.TV_SHOWS)
		plugintools.set_view(plugintools.TV_SHOWS)
	elif modo.upper() == "LIST":
		plugintools.set_view(plugintools.LIST)
		plugintools.set_view(plugintools.LIST)
		plugintools.set_view(plugintools.LIST)
	elif modo.upper() == "BIGLIST":
		plugintools.set_view(plugintools.BIGLIST)
		plugintools.set_view(plugintools.BIGLIST)
		plugintools.set_view(plugintools.BIGLIST)
	elif modo.upper() == "THUMBNAIL":
		plugintools.set_view(plugintools.THUMBNAIL)
		plugintools.set_view(plugintools.THUMBNAIL)
		plugintools.set_view(plugintools.THUMBNAIL)
	elif modo.upper() == "MOVIES":
		plugintools.set_view(plugintools.MOVIES)
		plugintools.set_view(plugintools.MOVIES)
		plugintools.set_view(plugintools.MOVIES)
	elif modo.upper() == "EPISODES":
    ...
    while i <= num_items:
        ...
        if data.replace(" ","").startswith("#EXTINF:-1") == True or data.replace(" ","").startswith("#EXTINF:1") == True or data.replace(" ","").startswith("#EXTINF:0") == True :  # Fixed by DMO: Soporte de listas con cualquier dígito tras #EXTINF y sin coma
            ...
            if data != "":  # Iniciamos análisis de la URL...
                ...
                if url.startswith("modovista") == True:  # Fixed by DarioMO: Para poder ordenar desde una lista m3u, el modo de vista en que quieres q se vea
                    url = url.replace("modovista:", "")
                    cambia_vista_m3u(url)
                    url = ""
                ...
                if url == "#multi" or url == "#multilink":
                    ...
                    if cat != "":  # Elementos con subcategoría de canales
                        ...
                        if server != "unknown":
                            ...
                            if url.find("diskokosmiko") >= 0 or url.find("copiapop") >= 0:
                                ...
                                plugintools.runAddon(action="runPlugin" , title = '[COLOR red][I]' + cat + ' / [/I][/COLOR][COLOR white] ' + title + '[COLOR lightyellow] [I]['+server+'][/I][/COLOR]' , url = url_montada , thumbnail = thumbnail , info_labels = datamovie , fanart = fanart , folder = False , isPlayable = True )
                            else:  #*******DMO*********
                    ...
                    # Sin categoría de canales
                        ...
                        if server != "unknown":
                            ...
                            if url.find("diskokosmiko") >= 0 or url.find("copiapop") >= 0:
                                url_montada = lanza_kosmipop(url=url)
                                plugintools.runAddon(action="runPlugin" , title = '[COLOR white]' + title + ' [COLOR lightyellow][I]['+server+'][/I][/COLOR]', url = url_montada , thumbnail = thumbnail , info_labels = datamovie, fanart = fanart , folder = False , isPlayable = True )
                            else:  #*******DMO*********
                ...
                if data.startswith("#") == True:
                    ...
                    if url.startswith("yt_playlist") == True:
                        ...
                        if server != "unknown":
                            ...
                            if url.find("diskokosmiko") >= 0 or url.find("copiapop") >= 0:
                                url_montada = lanza_kosmipop(url=url)
                                plugintools.runAddon(action="runPlugin" , title = title + ' [COLOR lightyellow]['+server+'][/COLOR]' , url = url_montada , info_labels = datamovie , thumbnail = thumbnail , fanart = fanart , folder = False , isPlayable = True)
                            else:  #*******DMO*********
    ...
    for entry in matches:
        if entry.find("isHost") <= 0:
            ...
            if server != "unknown":
                ...
                if url.find("diskokosmiko") >= 0 or url.find("copiapop") >= 0:
                    url_montada = lanza_kosmipop(url=url)
                    plugintools.runAddon(action="runPlugin" , title = title + ' [COLOR lightyellow]['+server+'][/COLOR]' , url = url_montada , fanart = fanart , thumbnail = thumbnail , folder = False , isPlayable = True )
                else:  #*******DMO*********
